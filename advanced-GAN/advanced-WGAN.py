...
def train_wgan_gp(epochs=300, batch_size=64, latent_dim=128, n_critic=5, 
               gp_weight=10.0, save_interval=10):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % save_interval == 0:
            ...
            if epoch % (save_interval * 5) == 0:
                generator.save(f"saved_models/wgan_generator_epoch_{epoch}.keras")
                critic.save(f"saved_models/wgan_critic_epoch_{epoch}.keras")
            ...
            plt.ylabel("Loss")
            plt.legend()
            plt.savefig(f"generated_images/wgan_loss_epoch_{epoch}.png")
            plt.close()

    # Save final models
    generator.save("saved_models/wgan_final_generator.keras")
    critic.save("saved_models/wgan_final_critic.keras")
